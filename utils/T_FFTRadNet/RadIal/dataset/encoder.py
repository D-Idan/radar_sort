    ...
    def encode(self,labels):
        ...
        for lab in labels:
            ...
            if(self.geometry['size']==1):
                ...
                print(
                    f"Target slice shape: {map[1, range_bin - s:range_bin + (s + 1), angle_bin - s:angle_bin + (s + 1)].shape}")
                print(
                    f"Source array shape: {((px_r + range_mod) - self.statistics['reg_mean'][0]) / self.statistics['reg_std'][0]}.shape")
                print(f"range_bin: {range_bin}, s: {s}")
                print(f"range_bin - s: {range_bin - s}, range_bin + (s + 1): {range_bin + (s + 1)}")
                print(f"angle_bin - s: {angle_bin - s}, angle_bin + (s + 1): {angle_bin + (s + 1)}")
